...
function initFields() {
  // body...
  for (var i = 6; i < 10; i++) {
    grinchHouse[grinchHouseIndex] = [i, 9];
    grinchHouseIndex++;
  }
...
function staticArrayContains(cord) {
  ...
  for (var i = 0; i < staticGrids.length; i++) {
    if (x === staticGrids[i][0] && y === staticGrids[i][1]) {
      return true;
    }
  }
  return false;
...
function grinchHouseContains(cord) {
  var x = cord[0];
  var y = cord[1];
  for (var i = 0; i < grinchHouse.length; i++) {
    if (x === grinchHouse[i][0] && y === grinchHouse[i][1]) {
      return true;
    }
  }
  return false;
  ...
  switch (dir) {
    case UP:
      if (
        ...
        gridType != TOP_BOTTOM &&
        gridType != TOP_ONLY &&
        gridType != LEFT_TOP_RIGHT &&
        gridType != TOP_RIGHT_BOTTOM &&
        gridType != BOTTOM_LEFT_TOP
      ) {
        canMove = true;
      }
      break;
    ...
    case DOWN:
      if (
        gridType != LEFT_BOTTOM &&
        gridType != TOP_BOTTOM &&
        gridType != RIGHT_BOTTOM &&
        gridType != BOTTOM_ONLY &&
        gridType != RIGHT_BOTTOM_LEFT &&
        gridType != BOTTOM_LEFT_TOP &&
        gridType != TOP_RIGHT_BOTTOM
      ) {
        canMove = true;
...
function welcomeScreen() {
  ...
  ctx.fillText(
    "DEVELOPED BY: ZI WANG, BINGYING XIA",
    CANVAS_WIDTH / 2,
    (CANVAS_HEIGHT / 20) * 19
  );
...
function updateCanvas() {
  ...
  if (gameOver() === true) {
    ...
    if (life > 0) {
      ...
      for (var i = 0; i < grinchs.length; i++) {
        fixGrids(grinchs[i].x, grinchs[i].y);
      }
      run();
    } else {
      clearInterval(intervalId);
    ...
    if (weakCounter > 0 && weakCounter < 2000 / timerDelay) {
      for (var i = 0; i < grinchs.length; i++) {
        grinchs[i].isBlinking = !grinchs[i].isBlinking;
      }
    }
    if (weakCounter > 0) {
      weakCounter--;
    ...
    if (weakCounter === 0) {
      for (var i = 0; i < grinchs.length; i++) {
        grinchs[i].isDead = false;
        grinchs[i].isWeak = false;
        grinchs[(i.isBlinking = false)];
        weakBonus = 200;
      }
    }
...
function eatBean() {
  if (onGridCenter(mrPacman.x, mrPacman.y)) {
    if (maze[mrPacman.getRow()][mrPacman.getCol()].beanType === NORMAL_BEAN) {
      ...
      maze[mrPacman.getRow()][mrPacman.getCol()].beanType === POWER_BEAN
    ) {
      score += parseInt(50);
      showScore();
      beansLeft--;

      //grinchs enter weak mode
      for (var i = 0; i < grinchs.length; i++) {
        grinchs[i].isWeak = true;
      }
...
//try to eat a weak grinch
function eatgrinch() {
  for (var i = 0; i < grinchs.length; i++) {
    if (
      Math.abs(mrPacman.x - grinchs[i].x) <= 5 &&
      Math.abs(mrPacman.y - grinchs[i].y) <= 5 &&
      grinchs[i].isWeak &&
      !grinchs[i].isDead
    ) {
      ...
      weakBonus *= 2;
      showScore();
      grinchs[i].isDead = true;
      grinchs[i].togrinchHouse();
    }
  }
}
...
//run the game. Create mrPacman and 4 grinchs. Reset their positions.
function run(isGodMode) {
  showScore();

  mrPacman = new Pacman(
    pacmanStartLoc[1] * GRID_WIDTH + GRID_WIDTH / 2,
    pacmanStartLoc[0] * GRID_HEIGHT + GRID_HEIGHT / 2,
    RIGHT
  );
  if (isGodMode === undefined || !isGodMode) {
    blinky = new grinch(0, 0, RED, DOWN);
    inky = new grinch(0, 0, CYAN, DOWN);
    pinky = new grinch(0, 0, PINK, DOWN);
    clyde = new grinch(0, 0, ORANGE, DOWN);

    blinky.togrinchHouse();
    inky.togrinchHouse();
    pinky.togrinchHouse();
    clyde.togrinchHouse();

    grinchs = [blinky, inky, pinky, clyde];
