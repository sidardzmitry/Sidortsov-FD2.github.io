function grinch(xCord, yCord, gColor, direction) {
  this.x = xCord;
  this.y = yCord;
  this.color = gColor;
  this.dir = direction;
  this.isWeak = false;
  this.radius = grinch_RADIUS;
  this.isMoving = false;
...
//send this grinch back to grinch house.
//location in grinch house is determined by its color
grinch.prototype.togrinchHouse = function () {
  var initX, initY;
  switch (this.color) {
    case ORANGE:
      initX = grinchHouse[0][1] * GRID_WIDTH + GRID_WIDTH / 2;
      initY = grinchHouse[0][0] * GRID_WIDTH + GRID_WIDTH / 2;
      break;

    case CYAN:
      initX = grinchHouse[1][1] * GRID_WIDTH + GRID_WIDTH / 2;
      initY = grinchHouse[1][0] * GRID_WIDTH + GRID_WIDTH / 2;
      break;

    case PINK:
      initX = grinchHouse[2][1] * GRID_WIDTH + GRID_WIDTH / 2;
      initY = grinchHouse[2][0] * GRID_WIDTH + GRID_WIDTH / 2;
      break;

    case RED:
      initX = grinchHouse[3][1] * GRID_WIDTH + GRID_WIDTH / 2;
      initY = grinchHouse[3][0] * GRID_WIDTH + GRID_WIDTH / 2;
      break;
  }
  this.x = initX;
  this.y = initY;
...
grinch.prototype.draw = function () {
  if (!this.isDead) {
    // body color
    if (this.isWeak) {
      if (this.isBlinking) {
    ...
    if (!this.isMoving) {
      ...
      ctx.lineTo(
        this.x - this.radius + this.radius / 3,
        this.y + this.radius - this.radius / 4
      );
      ctx.lineTo(
        this.x - this.radius + (this.radius / 3) * 2,
        this.y + this.radius
      );
  ...
  if (this.isWeak) {
    ...
    switch (this.dir) {
      case UP:
        ...
        ctx.arc(
          this.x - this.radius / 3,
          this.y - this.radius / 5 - this.radius / 6,
          this.radius / 6,
          0,
          Math.PI * 2,
          true
      ...
      case LEFT:
        ...
        ctx.beginPath();
        ctx.arc(
          this.x - this.radius / 3 - this.radius / 5,
          this.y - this.radius / 5,
          this.radius / 6,
          0,
          Math.PI * 2,
        ...
        ctx.beginPath();
        ctx.arc(
          this.x + this.radius / 3 - this.radius / 15,
          this.y - this.radius / 5,
          this.radius / 6,
          0,
          Math.PI * 2,
...
grinch.prototype.getRow = function () {
  return getRowIndex(this.y);
};

grinch.prototype.getCol = function () {
  return getColIndex(this.x);
};

//move one step in the current direction if allowed
grinch.prototype.moveOneStep = function () {
  // body...
  var newX = 0;
  var newY = 0;
...
grinch.prototype.turnBack = function () {
  this.dir = oppositeDir(this.dir);
};

//try to turn(if necessary) and move the grinch
grinch.prototype.move = function () {
  this.isMoving = !this.isMoving; //so the grinch looks like it's moving
  if (this.isWeak) {
    //if weak, reduce speed and make an immediate turn.
    //grinch starts making random moves until turning back to normal
    this.speed = speed / 2;
...
grinch.prototype.blinkyMove = function () {
  this.moveToPacman(true);
};

//pinky chooses the tile that is 4 steps ahead of pacman
grinch.prototype.pinkyMove = function () {
  this.moveToPacman(false);
};

//inky is unpredictable, makes random move
grinch.prototype.inkyMove = function () {
  this.randomMove();
};

grinch.prototype.moveToPacman = function (targetPacman) {
...
//get the distance from this grinch to pacman as if it moved one step in the given direction
grinch.prototype.getTestDistance = function (dir, targetPacman) {
  var toReturn = 0;
  this.dir = dir;
  this.moveOneStep();
  if (targetPacman) {
    toReturn = Math.sqrt(
      Math.pow(this.x - mrPacman.x, 2) + Math.pow(this.y - mrPacman.y, 2)
    );
  } else {
    switch (mrPacman.dir) {
      case LEFT:
        toReturn = Math.sqrt(
          Math.pow(this.x - (mrPacman.x - 4 * GRID_WIDTH), 2) +
            Math.pow(this.y - mrPacman.y, 2)
        );
        break;
      ...
      case UP:
        toReturn = Math.sqrt(
          Math.pow(this.x - mrPacman.x, 2) +
            Math.pow(this.y - (mrPacman.y - 4 * GRID_HEIGHT), 2)
        );
        break;
...
grinch.prototype.randomMove = function () {
  var nextDir = parseInt(Math.random() * 4) + 1;
  while (true) {
    if (nextDir != oppositeDir(this.dir) && canMove(this.x, this.y, nextDir)) {
      break;
    }
